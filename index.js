...
const euURLRegex =
  /https?:\/\/(([^\s]*)\.)?amzn\.([a-z.]{2,5})(\/d\/([^\s]*)|\/([^\s]*)\/?(?:d|o|gp|-)\/)(aw\/d\/|product\/)?(B[0-9]{2}[0-9A-Z]{7}|[0-9]{9}(?:X|[0-9]))([^\s]*)/gi;
...
function getASINFromFullUrl(url) {
  const match = euURLRegex.exec(url);
  ...
  try {
    ...
    if (
      ...
      if (raw_links) {
        ...
        euURLRegex.lastIndex = 0;

        while ((match = euURLRegex.exec(msg.text)) !== null) {
          const asin = match[8];
          const fullURL = match[0];
          replacements.push({ asin: asin, fullURL: fullURL });
        }
...
while ((match = shortURLRegex.exec(msg.text)) !== null) {
        const shortURL = match[0];
        euURLRegex.lastIndex = 0; // Otherwise sometimes getASINFromFullUrl won't succeed
        const url = await getLongUrl(shortURL);
    ...
    log(
      "ERROR, please file a bug report at @ilCatch"
    );
    console.log(e);
  }
});
